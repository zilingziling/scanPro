...
class Index extends Component {
  ...
  componentDidMount(){
    ...
    // 获取操作信息等
    api.post('deviceapi/base_teacher/return_applet',{
      address: 2,
      version
    }).then(response=>{
        ...
        if (res.code) {
          ...
          api.get('wechatmanage/ytjOpenId/findPlatformNoByCode',{
            ...
            //     获取个人操作信息
              wx.setStorageSync('openId',r.data.data.openId)
              // 平台描述
              wx.setStorageSync('description',r.data.data.description)
              // 存平台号
  ...
  render() {
    ...
    <View className='indexContent'>
      ...
      <View className='contentWrap'>
        <View className='blue' onClick={()=>Taro.navigateTo({
          url: "/pages/scan/scan"
        })}>
          <Text className='text'>{scan_open}</Text>
        </View>
        <View className='green' onClick={()=>Taro.navigateTo({
          url: "/pages/breakdown/breakdown"
        })}>
          <Text className='text'>{fault}</Text>
        </View>
      </View>
    </View>
    )
